...
fn decode_stats(deliver_tx: &DeliverTx) -> anyhow::Result<CreditStats> {
    ...
    fvm_ipld_encoding::from_slice::<recall_fendermint_actor_blobs_shared::params::GetStatsReturn>(&data)
        .map(|v| v.into())
...
fn decode_approve(deliver_tx: &DeliverTx) -> anyhow::Result<Approval> {
    let data = decode_bytes(deliver_tx)?;
    fvm_ipld_encoding::from_slice::<recall_fendermint_actor_blobs_shared::state::CreditApproval>(&data)
        .map(|v| v.into())
